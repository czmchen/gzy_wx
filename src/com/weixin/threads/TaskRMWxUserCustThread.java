...
public class TaskRMWxUserCustThread implements Runnable {

	private final static Logger logger = Logger.getLogger(TaskRMWxSendSKInfoThread.class);
		...
		while (true) {
			try {
				...
				if (lstRMWxUserCust.size() > 0) {
					updateOrderStatus(lstRMWxUserCust);// 先更新微信端
	...
	private void updateOrderStatus(List<RMWxUserCust> lstRMWxUserCust) {
	...
	private List<RMWxUserCust> getWxUserCust() throws RuntimeException {
		...
		try {
			conn = C3P0DBManager.getConnection();
			ps = conn.prepareStatement("select openid,mphone,CustRemark,Custid,CustKHWXH,CustName,crm_area,crm_addr_name,crm_province,crm_city,crm_town,KHMC,KHLXR,cityPickerCN from WxUserCust where issys = 1");
			rs = ps.executeQuery();
			while (rs.next()) {
