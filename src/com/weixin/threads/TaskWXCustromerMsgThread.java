...
public class TaskWXCustromerMsgThread implements Runnable {

	private final static Logger logger = Logger.getLogger(TaskWXCustromerMsgThread.class);
	...
	public void run() {
		while (true) {
			try {
				Thread.sleep(60000);
				logger.debug(new SimpleDateFormat("yyyy-MM-dd HH:mm:sss").format(new Date())
						+ " TaskWXCustromerMsgThread任务进行中。。。");
