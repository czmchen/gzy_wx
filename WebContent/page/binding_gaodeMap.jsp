...
<script type="text/javascript">
	...
	AMap.plugin('AMap.Geolocation', function() {
	    ...
	    geolocation.getCurrentPosition(function(status,result){
	        if(status=='complete'){
	            onComplete(result);
	            nowLocationMarker = new AMap.Marker({
		            icon: '${pageContext.request.contextPath}/page/images/marker_now_location.png',
		            position: [result.position.lng,result.position.lat],
		            offset: new AMap.Pixel(-13, -30)
		        });
	            nowLocationMarker.setMap(map);
		        
		        //marker 点击时打开
		        nowLocationMarker.on('click', function(e) {
		        	openNowLocationInfo(e);
		        });
	        }else{
	            onError(result)
	        }
	...
	function initNowLocationMarker(data){
    ...
    function mapSearch(searchKey){
    	AMap.plugin(['AMap.PlaceSearch'], function(){
			...
			placeSearch.setPageSize(50);
	     	...
	     	placeSearch.searchNearBy(searchKey, cpoint, 5000, function(status, result) {
	...
	function sleep(ms) {
	 ...
	 //在指定位置打开信息窗体
    function openNowLocationInfo(e) {
    	var contentVal = "<div style='text-align: left;width:200px;height: 50px;'><div style='width: 10px;height: 13px;'></div><div><a href='javascript:void(0);' style='margin-right: 10px;' onclick='settingNowLocation(&apos;"+e.target.getExtData().id+"&apos;);'><img alt='mark' src='${pageContext.request.contextPath}/page/distribution/images/mark_32.png'>设置当前位置</a></div><div style='width: 80px;'></div></div>";
        //构建信息窗体中显示的内容
    ...
    function settingNowLocation(){
    	window.parent.$.confirm({
			  title: '位置选取服务',
			  text: '注意：该位置是未关联高德地图商户,是否确认选取当前位置为精准送货地址',
			  onOK: function () {
				 $('#latitude',parent.document).val(nowLocationMarker.getPosition().lat);
   		         $('#longitude',parent.document).val(nowLocationMarker.getPosition().lng);
   		         $('#gaodeId',parent.document).val('');
 		         $('#merchantsId',parent.document).val('');
				 window.parent.$.closePopup();
				 window.parent.$.alert("设置成功,请提交更新！");
			  },
			  onCancel: function () {
			  }
			});
  ...
  //在指定位置打开信息窗体
    function openFindInfo(e) {
    	...
    	var contentVal = "<div class='input-card'>"
    				  ...
    				  + "<span><a href='javascript:void(0);' style='margin-right: 10px;' onclick='settingMerchantLocation(&apos;"+e.target.getExtData().id+"&apos;);'>设置当前商户<img alt='mark' src='${pageContext.request.contextPath}/page/distribution/images/mark_32.png'></a></span><span width='10px'></span>"
    	contentVal += "</div></br>"
    		 		+ htmlContent
    				+ "<div>";
        //构建信息窗体中显示的内容
    	...
    	window.parent.$.confirm({
			  ...
			  onOK: function () {
				  var orgMarker = allMarkerData.get("Find_"+mapIdVal);
					$.ajax({
						url:'${pageContext.request.contextPath}/page/userMerchants.html',
					    type: 'POST',
					    data: orgMarker.getExtData().par,
						success:function(rs){
							infoWindow.close();
							if(rs!='error'&&rs!=null&&rs!=''&&rs!='null'&&rs!='undefined'&&rs!=undefined){
							    $('#latitude',parent.document).val(orgMarker.getExtData().par.lat);
				   		        $('#longitude',parent.document).val(orgMarker.getExtData().par.lng);
				   		        $('#gaodeId',parent.document).val(mapIdVal);
				   		        $('#merchantsId',parent.document).val(rs);
								window.parent.$.closePopup();
